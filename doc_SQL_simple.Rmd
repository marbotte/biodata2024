...
fileSQLCommands<-"doc_SQL_simple.sql"
...
download_link(
  link = paste0("https://raw.githubusercontent.com/marbotte/cursoBiodata/master/download/SQL_code/documentation/",fileSQLCommands),
...
Voy a utilizar principalmente PostgreSQL, pero cuando existan diferencias, mostraré también la versión SQLite.
...
```{sql, select_example_1,connection=pg_biodata1,code=extractSQLquery("./sql_code/doc_SQL_simple.sql","select_example_1")}
```
...
```{sql, select_estrella,connection=pg_biodata1,code=extractSQLquery("./sql_code/doc_SQL_simple.sql","select_estrella")}
```
...
```{sql, case1,connection=pg_biodata1,code=extractSQLquery("./sql_code/doc_SQL_simple.sql","case1")}
```
...
```{sql, quote1,connection=pg_biodata1,code=extractSQLquery("./sql_code/doc_SQL_simple.sql","quote1")}
```

Sin embargo, todo como funciona como si el sistema pasaría todo a minúsculas cuando no utilizamos las comillas.

```{sql, case2,connection=sl_biodata1,code=extractSQLquery("./sql_code/doc_SQL_simple.sql","case2")}
```
...
```{sql, colnames1,connection=sl_biodata1,code=extractSQLquery("./sql_code/doc_SQL_simple.sql","colnames1")}
```

Realmente, la clausula `AS` es facultativa:


```{sql, colnames2,connection=sl_biodata1,code=extractSQLquery("./sql_code/doc_SQL_simple.sql","colnames2")}
```
...
```{sql, colnames2_2,connection=pg_biodata1,code=extractSQLquery("./sql_code/doc_SQL_simple.sql","colnames2")}
```
...
```{sql, select_anadir_num,connection=pg_biodata1,code=extractSQLquery("./sql_code/doc_SQL_simple.sql","select_anadir_num")}
```
...
```{sql, select_anadir_texto,connection=pg_biodata1,code=extractSQLquery("./sql_code/doc_SQL_simple.sql","select_anadir_texto")}
```

### Añadir una columna de calculo

```{sql, calculo1,connection=pg_biodata1,code=extractSQLquery("./sql_code/doc_SQL_simple.sql","calculo1")}
```
...
La clausula `DISTINCT` permite tomar en cuenta solo una vez cada valor:

```{sql, connection=pg_biodata1,code=extractSQLquery("./sql_code/doc_SQL_simple.sql","distinct1")}
```

Como lo pueden ver, "Canis lupus" ya no está repetido en esta lista
(anotar, si ponen varias variables en esta consulta, filtrará las asociaciones de variables que son unicas...)



Se puede manejar de manera más fina: podemos decidir mostrar varias variables, pero considerando únicamente valores únicos para una variable.
La fila que aparece en esos casos es la primera fila en el orden decidido por `ORDER BY`.
Anotar que las variables en `DISTINCT ON` deben ser repetida y en primera posición en `ORDER BY`.

```{sql, connection=pg_biodata1,code=extractSQLquery("./sql_code/doc_SQL_simple.sql","distinct2")}
```

```{sql, connection=pg_biodata1,code=extractSQLquery("./sql_code/doc_SQL_simple.sql","distinct3")}
```

La clausula `DISTINCT` puede también estar utilizada dentro de otras funciones, con `COUNT`, permite enumerar los casos diferentes:

```{sql, connection=pg_biodata1,code=extractSQLquery("./sql_code/doc_SQL_simple.sql","distinct4")}
```

Anotar, sin `DISTINCT`, la misma consulta cuenta todos los casos, incluso no únicos:


```{sql, connection=pg_biodata1,code=extractSQLquery("./sql_code/doc_SQL_simple.sql","undistinct4")}
```

De la misma manera, `DISTINCT` funciona con ARRAY_AGG y muchas otras funciones

```{sql, connection=pg_biodata1,code=extractSQLquery("./sql_code/doc_SQL_simple.sql","distinct5")}
```

Sin `DISTINCT`:

```{sql, connection=pg_biodata1,code=extractSQLquery("./sql_code/doc_SQL_simple.sql","undistinct5")}
```


# CASE
Es posible utilizar la clausula `CASE` para tratar de manera diferenciada, según una condición, los casos de cada fila:


```{sql, connection=pg_biodata1,code=extractSQLquery("./sql_code/doc_SQL_simple.sql","case_1")}
```

Cada uno de los casos `WHEN` están leído hasta que uno valga `TRUE`, el primero encontrado envía el valor que está en `THEN`, si ninguno está encontrado entonces es el valor de `ELSE` que está enviado.
Poner un valor en `ELSE` no es obligatorio: por defecto, vale `NULL`.
Para tener más información sobre `CASE`, ver <https://www.postgresql.org/docs/current/functions-conditional.html#FUNCTIONS-CASE> para postgreSQL y <https://www.sqlite.org/lang_expr.html> (parte 7) para SQLite.
